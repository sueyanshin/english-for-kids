...
(function () {
  ...
  Handlebars.registerHelper("progressicon", (status, options) => {
    ...
    return new Handlebars.SafeString(
      `<i class="${icons[status]}" aria-hidden="true"></i>`
    );
  });
  ...
  document.querySelector(".main").addEventListener("keydown", (e) => {
    ...
    if (
      event.keyCode === 13 &&
      (el.classList.contains("side1") || el.classList.contains("side2"))
    ) {
  ...
  function select() {
    ...
    Render.header(false, "My Flashcards");
  ...
  function train(name) {
    ...
    if (autocheck) {
      document
        ...
        .addEventListener("click", drawNextCard);
    } else {
      document.getElementById("nextButtons").addEventListener("click", (e) => {
        ...
        if (el.id === "wrongAnswer" || el.id === "correctAnswer") {
          const submission =
            el.id === "correctAnswer"
              ? flashcards.revealAnswer().answers[0]
              : "";
  ...
  function edit(name) {
    ...
    Render.editing(
      flashcards.exposeDeck().cards,
      flashcards.deckLength(),
      name,
  ...
  function drawNextCard() {
    recordProgress();
    let card = numToRetry
      ? flashcards.draw(cardsToRetry.splice(0, 1)[0])
      : flashcards.drawNext();
    if (!card) {
      ...
      document
        .querySelector(".answer__input")
        .addEventListener("keydown", enterAnswer);
      document
        .querySelector("#card")
        .removeEventListener("click", drawNextCard);
  ...
  function submitAnswer() {
    ...
    if (usersettings.autocheck) {
      const result = flashcards.checkAnswer(userAnswer.value.trim()),
        answers = usersettings.firstanswer
          ? [result.answers[0]]
          : result.answers;
  ...
  const Render = {
    ...
    question: function (qText, diff, isFlipped) {
      let context = {
        ...
        userAnswer = document.querySelector(".answer__input");
      document.querySelector(".card__side--question").innerHTML =
        questionTemplate(context);
      //animate card flip
    ...
    answer: function (answers, diff, outcome) {
      ...
      document.querySelector(".card__side--answer").innerHTML =
        answerTemplate(context);
      //animate card flip
    ...
    progress: function (sessionInfo, totalCards) {
      let bars = [],
        ...
        cardsRemaining = numToRetry
          ? numToRetry - cardsAnswered
          : totalCards - cardsAnswered,
        i;
      ...
      document.querySelector(".progress").innerHTML = progressTemplate({
        bars: bars,
      });
    },
